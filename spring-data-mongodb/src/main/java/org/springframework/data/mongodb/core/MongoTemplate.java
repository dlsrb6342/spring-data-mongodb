	...
	protected Object saveDocument(String collectionName, Document dbDoc, Class<?> entityClass) {
		...
		return execute(collectionName, collection -> {
			...
			if (!mapped.hasId()) {
				...
				filter = !updateContext.requiresShardKey(filter, entity) ? filter
						: updateContext.applyShardKey(entity, filter,
	...
	protected UpdateResult doUpdate(String collectionName, Query query, UpdateDefinition update,
		...
		return execute(collectionName, collection -> {
			...
			if (!UpdateMapper.isUpdateObject(updateObj)) {
				...
				filter = !updateContext.requiresShardKey(filter, entity) ? filter
						: updateContext.applyShardKey(entity, filter, collection.find(filter, Document.class).projection(updateContext.getMappedShardKey(entity)).first());
