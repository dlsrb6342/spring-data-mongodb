	...
	protected Mono<Object> saveDocument(String collectionName, Document document, Class<?> entityClass) {
		...
		return createMono(collectionName, collection -> {
			...
			if (!mapped.hasId()) {
				...
				Mono<Document> theFilter = !updateContext.requiresShardKey(filter, entity) ? Mono.just(filter)
						: Mono.from(collection.find(filter, Document.class).projection(updateContext.getMappedShardKey(entity)).first()).defaultIfEmpty(replacement)
